...
export function CreatorSignup(props: Readonly<{
  onBack: () => void
  onSubmit: (payload: {
    displayName: string
    email: string
  ...
  const [touched, setTouched] = useState<{ displayName: boolean; email: boolean; handle: boolean }>({
    displayName: false,
    email: false,
    handle: false,
  })
  ...
  const handleValid = useMemo(() => /^@[a-zA-Z0-9_.]{3,20}$/.test(handle), [handle])
  ...
  return (
    <view className='Signup'>
      <view className='SignupHeader'>
        ...
        <text className='SignupSubtitle'>Earn fairly in real time from engaged viewers</text>
      </view>
      ...
      <view className='FormCard'>
        <view className='FormField'>
          ...
          <input
            ...
            bindblur={() => setTouched((v: { displayName: boolean; email: boolean; handle: boolean }) => ({ ...v, displayName: true }))}
          />
          {touched.displayName && !nameValid && <text className='FormError'>Please enter your display name</text>}
        </view>
        ...
        <view className='FormField'>
          ...
          <input
            ...
            bindblur={() => setTouched((v: { displayName: boolean; email: boolean; handle: boolean }) => ({ ...v, email: true }))}
          />
        ...
        <view className='FormField'>
          ...
          <input
            ...
            value={handle}
            bindinput={(e: any) => setHandle(e.detail?.value ?? e.target?.value ?? '')}
            bindblur={() => setTouched((v: { displayName: boolean; email: boolean; handle: boolean }) => ({ ...v, handle: true }))}
          />
          {touched.handle && !handleValid && (
            <text className='FormError'>Handle must start with @ and be 3–20 characters</text>
          )}
        </view>
        ...
        <view className='FormField'>
          ...
          <input
            ...
            value={niche}
            bindinput={(e: any) => setNiche(e.detail?.value ?? e.target?.value ?? '')}
          />
          <text className='FormHint'>We use this to improve recommendations</text>
        </view>
